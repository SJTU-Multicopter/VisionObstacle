...
#include "../../../devel/include/px4_autonomy/Velocity.h"
#include "../../../devel/include/px4_autonomy/Position.h"
...
int square_nw, square_nh;
int slade_square_thred;
int body_square_min;
int body_square_max;
...
Vector4f &plane_ransac(MatrixXf &square, Vector4f &paras, Vector3f &plane_vec, Vector3f &plane_point, int &pixel_counter, int iterations = 5, float dist_filter = 0.2f, float ratio = 1.f) //Use RANSAC to fit planes. MatrixXf square(STEP*STEP, 3), (X,Y,Z) for each pixel
{
    ...
    for(int i = 0; i < iterations; i++)
    {
        while(1)
        {
          ...
          //cout<<n<<endl;
          n1 = (int)((n(0) + 1) / 2 * (pixel_counter - 1));
          n2 = (int)((n(1) + 1) / 2 * (pixel_counter - 1));
          n3 = (int)((n(2) + 1) / 2 * (pixel_counter - 1));
...
int main(int argc, char** argv)
{
    ...
    slade_square_thred = square_nh * 0.2;
    body_square_min = square_nw * 0.15;
    body_square_max = square_nw - body_square_min;
    depth_mtr = MatrixXf::Zero(HEIGHT, WIDTH);
    init_flag = true;
